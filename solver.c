...
/*   Updated: 2018/12/27 00:39:36 by amerlon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_placable(t_map m, int c, t_trio trio)
{
	...
	{
		if ((c % len == len - 1|| ((fig.c1) % len == len - 1) || ((fig.c2) % len == len - 1)
			|| ((fig.c3) % len == len - 1)) && ((c % len == 0 || (fig.c1) % len == 0) ||
				((fig.c2) % len == 0) || ((fig.c3) % len == 0)))
			return (0);
		else
			return (1);
	}
...
void	solver(int *tetriminos, t_trio *masks)
{
	...
	// while (*tetriminos != -1)
	// {
	// 	printf("(%d, %d, %d)\n", masks[*tetriminos].c1, masks[*tetriminos].c2, masks[*tetriminos].c3);
	// 	tetriminos++;
	// }
}
